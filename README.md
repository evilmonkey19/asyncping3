...
![PyPI - Downloads](https://img.shields.io/pypi/dm/asyncping.svg)
...
> The Python3 version originally from [here](http://github.com/kyan001/asyncping).\
...
```sh
pip install asyncping
```

```python
>>> from asyncping import ping, verbose_ping
...
```sh
pip install asyncping  # install asyncping
pip install --upgrade asyncping  # upgrade asyncping
pip uninstall asyncping  # uninstall asyncping
```
...
```python
>>> import asyncping
>>> asyncping.DEBUG = True  # Default is False.

>>> asyncping.ping("example.com")  # "ping()" prints received IP header and ICMP header.
...
```python
>>> import asyncping
>>> asyncping.EXCEPTIONS = True  # Default is False.

>>> asyncping.ping("example.com", timeout=0.0001)  # All Exceptions are subclasses of PingError
[... Traceback ...]
...
$ pping -v  # -v/--version. Show asyncping version number.
3.0.0
